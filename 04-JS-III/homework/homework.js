...
function todosIguales(arreglo) {
  ...
  if(arreglo.length>0){
    ...
    for (let i = 0; i < arreglo.length; i++) {
      if(arreglo[i]!=primero){bandera = false;}
    }
    return bandera;
  }else{
    return false;
...
function mesesDelAño(array) {
  ...
  var nuevoArray = new Array();
  for (let i = 0; i < array.length; i++) {
    if(array[i]==="Enero"){bandera1 = true;nuevoArray.push(array[i]);}else
    if(array[i]==="Marzo"){bandera2 = true;nuevoArray.push(array[i]);}else
    if(array[i]==="Noviembre"){bandera3 = true;nuevoArray.push(array[i]);}
  }
